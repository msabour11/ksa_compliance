...
def create_tax_categories(
    doc: SalesInvoice | PaymentEntry, item_lines: list, is_tax_included: bool
) -> dict:
    tax_category_map = frappe._dict()
    sales_taxes_and_charges_template = doc.get(
        get_right_fieldname("taxes_and_charges", doc.doctype)
    )
    item_tax_templates = [
        row.item_tax_template for row in item_lines if row.item_tax_template
    ]
    if sales_taxes_and_charges_template and not item_tax_templates:
        ...
        zatca_category = frappe.db.get_value(
            "Tax Category", tax_category_id, "custom_zatca_category"
        )
        ...
        tax_category = TaxCategory(
            zatca_tax_category_id=tax_category_id,
        ...
        tax_category_by_items = TaxCategoryByItems(
            tax_category=tax_category, items=[row for row in item_lines]
        )
    ...
    for row in item_lines:
        if not row.item_tax_template and sales_taxes_and_charges_template:
            ...
            tax_category_id = map_tax_category(
                item_tax_template_id=row.item_tax_template
            )
            tax_category_percent = frappe.db.get_value(
...
def check_item_tax_template(
    doc: SalesInvoice, item_lines: list, sales_taxes_and_charges_template: str
) -> None:
...
def _get_amounts(tax_category: TaxCategoryByItems) -> float:
    ...
    for row in tax_category.items:
        # round each value to 2 decimal places for ZATCA compliance
        taxable_amount += round(row.net_amount, 2)
        tax_amount += round(row.tax_amount, 2)
        total_discount += round(row.amount - row.net_amount, 2)

    # round final amounts to 2 decimal places
    amounts.taxable_amount = round(taxable_amount, 2)
    amounts.tax_amount = round(tax_amount, 2)
    amounts.total_discount = round(total_discount, 2)

    return amounts


def create_allowance_charge(
    doc: SalesInvoice | PaymentEntry, tax_total: frappe._dict
) -> list:
    allowance_charges = []
    ...
    if doc.doctype == "Sales Invoice" and doc.discount_amount:
        zatca_discount_reason = get_zatca_discount_reason_by_name(
            name=doc.custom_zatca_discount_reason
        )
    ...
    for row in tax_total.tax_subtotal:
        ...
        allowance_charge = AllowanceCharge(
            ...
            amount=round(row.total_discount, 2),  # round to 2 decimal places
        )
