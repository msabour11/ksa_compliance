...
from .prepayment_invoice.prepayment_invoice_factory import (
...
class Einvoice:
    def __init__(
        ...
        if self.business_settings_doc.enable_branch_configuration:
            if not self.sales_invoice_doc.branch:
                fthrow(
                    msg=ft(
                        "Branch is mandatory when ZATCA Branch configuration is enabled."
                    ),
        ...
        if self.sales_invoice_doc.get("is_debit_note") or self.sales_invoice_doc.get(
            "is_return"
        ):
    ...
    def get_float_value(
        ...
        field_value = cast(
            any, source_doc.get(get_right_fieldname(field_name, source_doc.doctype))
        )
    ...
    def get_time_value(self, field_name, source_doc, xml_name, parent) -> str | None:
        field_value = source_doc.get(
            get_right_fieldname(field_name, source_doc.doctype), None
        )
    ...
    def compute_invoice_discount_amount(self):
        ...
        if (
            self.sales_invoice_doc.apply_discount_on != "Grand Total"
        ...
        if applied_discount_percent == 0:
            applied_discount_percent = (
                discount_amount / (total_without_vat + tax_amount)
            ) * 100
        applied_discount_amount = total_without_vat * (applied_discount_percent / 100)
        self.result["invoice"]["allowance_total_amount"] = round(
            applied_discount_amount, 2
        )
        self.additional_fields_doc.fatoora_invoice_discount_amount = round(
            applied_discount_amount, 2
        )
    ...
    def get_business_settings_and_seller_details(self):
        ...
        self.get_text_value(
            ...
            source_doc=(
                self.branch_doc if has_branch_address else self.business_settings_doc
            ),
        ...
        self.get_text_value(
            field_name=(
                "custom_additional_street"
                if has_branch_address
                else "additional_street"
            ),
            source_doc=(
                self.branch_doc if has_branch_address else self.business_settings_doc
            ),
        ...
        self.get_text_value(
            field_name=(
                "custom_building_number" if has_branch_address else "building_number"
            ),
            source_doc=(
                self.branch_doc if has_branch_address else self.business_settings_doc
            ),
    ...
    def append_to_item_lines(
        self, item_lines: list, is_tax_included: bool, sales_invoice_doc: SalesInvoice
    ) -> None:
        """
        ...
        if sales_invoice_doc.doctype == "Payment Entry":
            ...
            self._append_sales_invoice_items(
                item_lines, is_tax_included, sales_invoice_doc
            )
    ...
    def _append_payment_entry_item(self, item_lines: list, doc: PaymentEntry) -> None:
        ...
        item_data = {
            ...
            "item_name": self.sales_invoice_doc.custom_prepayment_invoice_description
            or self.sales_invoice_doc.remarks,
            "net_amount": round(abs(values.net_amount), 2),
            "amount_after_discount": round(abs(values.amount_after_discount), 2),
            "amount": round(abs(values.amount), 2),
            "base_net_rate": round(abs(self.sales_invoice_doc.received_amount), 2),
            "base_net_amount": round(abs(self.sales_invoice_doc.received_amount), 2),
            "line_extension_amount": round(abs(values.line_extension_amount), 2),
            "base_amount": round(abs(self.sales_invoice_doc.received_amount), 2),
            "rounding_amount": round(abs(values.rounding_amount), 2),
            "rate": round(abs(self.sales_invoice_doc.received_amount), 2),
            "discount_percentage": 0.0,
            "discount_amount": 0.0,
            "item_tax_template": None,
            "tax_percent": self.sales_invoice_doc.taxes[0].rate,
            "tax_amount": round(abs(values.tax_amount), 2),
            "total_amount": round(
                abs(self.sales_invoice_doc.received_amount_after_tax), 2
            ),
        }
    ...
    def _calculate_payment_entry_values(self, doc: PaymentEntry) -> dict:
        ...
        if not doc.taxes:
            fthrow(
                msg=ft(
                    "Payment Entry $name does not have any taxes. Please add taxes to the Payment Entry.",
                    name=doc.name,
                )
            )
        ...
        if charge_type == "Actual":
            ...
            values.amount = abs(self.sales_invoice_doc.received_amount) - abs(
                tax_amount
            )
            values.net_amount = abs(self.sales_invoice_doc.received_amount) - abs(
                tax_amount
            )
    ...
    def _append_sales_invoice_items(
        self, item_lines: list, is_tax_included: bool, doc: SalesInvoice
    ) -> None:
        """Processes regular sales invoice items with proper tax and discount calculations."""
        for item in doc.items:
            has_discount = (
                isinstance(item.discount_amount, float) and item.discount_amount > 0
            )

            tax_percent = abs(item.tax_rate or 0.0)

            # Calculate amounts based on whether tax is included in price
            if is_tax_included:
                # When tax is included in the rate, extract the base amount first
                total_with_tax = abs(item.amount)
                amount_with_qty = round(total_with_tax / (1 + (tax_percent / 100)), 2)
                tax_amount_with_qty = round(total_with_tax - amount_with_qty, 2)
            else:
                # Tax-exclusive: use amounts as-is
                amount_with_qty = round(abs(item.amount), 2)
                tax_amount_with_qty = round(abs(item.tax_amount or 0.0), 2)

            discount_with_qty = (
                round(abs(item.discount_amount * item.qty), 2) if has_discount else 0.0
            )

            # Net amount is amount after discount
            net_amount_with_qty = (
                round(amount_with_qty - discount_with_qty, 2)
                if has_discount
                else amount_with_qty
            )

            rate_without_qty = round(abs(item.rate), 2)
            ...
            item_data = {
                ...
                "discount_percentage": (
                    abs(item.discount_percentage) if has_discount else 0.0
                ),
                "tax_percent": tax_percent,
                "amount": amount_with_qty,
                "rounding_amount": round(tax_amount_with_qty + amount_with_qty, 2),
                "base_amount": round(amount_with_qty + discount_with_qty, 2),
    ...
    def get_e_invoice_details(self, invoice_type: str):
        ...
        if self.sales_invoice_doc.get("is_debit_note") or self.sales_invoice_doc.get(
            ...
            additional_references = cast(
                ...
                self.sales_invoice_doc.get(
                    "custom_return_against_additional_references"
                ),
            )
            if additional_references:
                billing_references.extend(
                    [ref.sales_invoice for ref in additional_references]
                )
        ...
        is_tax_included = self.sales_invoice_doc.taxes[0].get(
            get_right_fieldname(
                "included_in_print_rate", self.sales_invoice_doc.doctype
            )
        )
        self.append_to_item_lines(item_lines, is_tax_included, self.sales_invoice_doc)
        tax_categories = create_tax_categories(
            self.sales_invoice_doc, item_lines, is_tax_included
        )
        ...
        if self.sales_invoice_doc.doctype == "Payment Entry":
            ...
            if charge_type != "Actual":
                self.result["invoice"]["base_total_taxes_and_charges"] = round(
                    abs(self.sales_invoice_doc.base_total_taxes_and_charges)
                    / (1 + tax_percent),
                    2,
                )
                self.result["invoice"]["total_taxes_and_charges"] = round(
                    abs(self.sales_invoice_doc.total_taxes_and_charges)
                    / (1 + tax_percent),
                    2,
                )
        ...
        # Calculate line_extension_amount from item lines (sum of amounts before tax)
        self.result["invoice"]["line_extension_amount"] = round(
            sum(it["amount"] for it in item_lines), 2
        )

        # Calculate discount amount
        self.compute_invoice_discount_amount()

        # Calculate net_total (line_extension_amount - allowance)
        net_total_calculated = round(
            self.result["invoice"]["line_extension_amount"]
            - self.result["invoice"]["allowance_total_amount"],
            2,
        )
        self.result["invoice"]["net_total"] = net_total_calculated

        # For Payment Entry, handle special case
        if self.sales_invoice_doc.doctype == "Payment Entry":
            if self.sales_invoice_doc.taxes[0].included_in_paid_amount:
                self.result["invoice"]["net_total"] = round(
                    self.sales_invoice_doc.paid_amount
                    - self.sales_invoice_doc.total_taxes_and_charges,
                    2,
                )

        # Recalculate tax amount based on net_total to ensure consistency
        if self.sales_invoice_doc.doctype != "Payment Entry" and tax_total.get(
            "tax_subtotal"
        ):
            tax_rate = tax_total["tax_subtotal"][0]["tax_category"]["percent"]
            recalculated_tax = round(
                self.result["invoice"]["net_total"] * (tax_rate / 100), 2
            )

            # Update tax totals with recalculated values
            self.result["invoice"]["total_taxes_and_charges"] = recalculated_tax
            self.result["invoice"]["base_total_taxes_and_charges"] = recalculated_tax
            self.result["invoice"]["tax_total"]["tax_amount"] = recalculated_tax
            self.result["invoice"]["tax_total"]["taxable_amount"] = self.result[
                "invoice"
            ]["net_total"]

            # Update tax subtotal as well
            if self.result["invoice"]["tax_total"]["tax_subtotal"]:
                self.result["invoice"]["tax_total"]["tax_subtotal"][0][
                    "tax_amount"
                ] = recalculated_tax
                self.result["invoice"]["tax_total"]["tax_subtotal"][0][
                    "taxable_amount"
        ...
        # Calculate grand_total (net_total + tax)
        self.result["invoice"]["grand_total"] = round(
            self.result["invoice"]["net_total"]
        ...
        if self.sales_invoice_doc.doctype != "Payment Entry":
            rounding_adjustment = round(self.sales_invoice_doc.rounding_adjustment, 2)

        self.result["invoice"]["payable_amount"] = round(
            self.result["invoice"]["grand_total"] + rounding_adjustment, 2
        )
        ...
        if self.sales_invoice_doc.doctype == "POS Invoice":
            return
        if (
            self.sales_invoice_doc.doctype == "Sales Invoice"
            and not self.sales_invoice_doc.advances
        ):
            return
        self.result["prepayment_invoice"] = prepayment_invoice_factory_create(
