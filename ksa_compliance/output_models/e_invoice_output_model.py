...
class Einvoice:
    ...
    def compute_invoice_discount_amount(self):
        ...
        self.additional_fields_doc.fatoora_invoice_discount_amount = (
    ...
    def _append_sales_invoice_items(
        ...
        for item in doc.items:
            ...
            discount_with_qty = (
                abs(item.discount_amount * item.qty) if has_discount else 0.0
            )
            amount_with_qty = abs(
    ...
    def get_e_invoice_details(self, invoice_type: str):
        ...
        self.result["invoice"]["allowance_total_amount"] = sum(
            ac.get("amount", 0) for ac in allowance_charge
        )
        ...
        self.result["invoice"]["line_extension_amount"] = sum(
            it["amount"] for it in item_lines
        )
        # self.compute_invoice_discount_amount()
        ...
        self.result["invoice"]["payable_amount"] = (
            self.result["invoice"]["grand_total"] + rounding_adjustment
        )
